...
//			System.out.println(name);
	...
	public void testShares_shareFolder(){
		...
		for(int i=0;i<fileops.fList.size();i++){
			...
			fileops.upLoadFile(file, folder_path, uid, paramsMap);
		}
		String url = fileops.jsonDecode(fileops.shares(folder_path, "false", uid), "url");
		String copy_ref = url.substring(url.lastIndexOf("/") + 1,url.length());
		paramsMap.put("copy_ref", copy_ref);
		String res = shareops.getShareObjecInfo(paramsMap, uid);
		System.out.println(res);
		JSONArray arr = new JSONArray().fromObject(fileops.jsonDecode(res, "contents"));
		List<String> fileList = new ArrayList<String>();
		for (int i = 0; i < arr.size(); i++) {
			String name = arr.getJSONObject(i).getString("name");
			for (int j = 0; j < fileops.fList.size(); j++) {
				if(fileops.fList.get(j).getName().equals(name)){
					fileList.add(name);
				}
			}
		}
		Assert.assertTrue(arr.size() == fileList.size(),"testShares_文件夹分享失败\n"
						+ CommonUtils.getRequestURL(vdiskRequest.getReqsInfo()));
	}
	@Test(description="空文件夹分享")
	public void testShares_emptyShareFolder(){
		String folder_path = vDirector + "/shareFolder_" + CommonUtils.getExpireTime(0);
		fileops.createFolder(folder_path, uid);
		String url = fileops.jsonDecode(fileops.shares(folder_path, "false", uid), "url");
		String copy_ref = url.substring(url.lastIndexOf("/") + 1,url.length());
		...
		System.out.println(res);
		JSONArray arr = new JSONArray().fromObject(fileops.jsonDecode(res, "contents"));
		Assert.assertTrue(arr.size() == 0,"testShares_空文件夹分享失败\n"
					+ CommonUtils.getRequestURL(vdiskRequest.getReqsInfo()));
	}
//	@Test(description="文件夹包含敏感词")
//	public void testShares_folder_restriction_40304(){
//		fileops.jsonDecode(fileops.createFolder(vDirector + "/" + "法轮功", uid),"path");
//		String res = fileops.shares(vDirector + "/" + CommonUtils.urlEncode("法轮功"), "", uid);
//		System.out.println(res);
//	}
